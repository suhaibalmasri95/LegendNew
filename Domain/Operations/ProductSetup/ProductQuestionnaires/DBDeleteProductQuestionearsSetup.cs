...
namespace Domain.Operations.ProductSetup.ProductQuestionnaires
{
   public static class DBDeleteProductQuestionearsSetup
    {
        public async static Task<IDTO> DeleteProductSubjectTypeAsync(ProductQuestionnaire product)
        {
        ...
        public async static Task<IDTO> DeleteMultipleProductSubjectTypeAsync(long[] IDs)
        {
            ...
            if (await NonQueryExecuter.ExecuteNonQueryAsync(MultiDeleteFormater.Format(typeof(ProductQuestionnaire), IDs)) == -1)
                complate.message = "Operation Successed";
            else
                complate.message = "Operation Failed";
