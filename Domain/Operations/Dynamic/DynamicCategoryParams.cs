using System;
using System.Collections.Generic;
using System.Text;

namespace Domain.Operations.Dynamic
{
   public static class DynamicCategoryParams
    {
        public const string PARAMETER_ID = "IN_ID";
        public const string PARAMETER_PRORDUCT_CAT_LEVEL = "IN_CAT_LEVEL";
        public const string PARAMETER_PRORDUCT_ST_LOB = "IN_ST_LOB";
        public const string PARAMETER_PRORDUCT_ST_SUB_LOB = "IN_ST_SUB_LOB";
        public const string PARAMETER_PRORDUCT_ST_CAT_ID = "IN_ST_CAT_ID";
        public const string PARAMETER_PRORDUCT_ID = "IN_ST_PRD_ID";
        public const string PARAMETER_PRORDUCT_DETAILS_ID = "IN_ST_PRDT_ID";
        public const string PARAMETER_REF_SELECT = "IN_REF_SELECT";
        public const string PARAMETER_LANG_ID = "IN_LANG";
    }
}
